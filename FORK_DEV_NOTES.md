...
`npm run validate:tools` imports the built `dist/tools/*.js` catalog, so run `npm run build` first. It validates the 272-tool catalog for duplicate names, snake_case names, handler shape, required-property references, and basic JSON-schema property metadata.
